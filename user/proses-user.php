...
if(isset($_POST['simpan'])){
	...
	if(mysqli_num_rows($cekUsername)){
		echo "<script>
				alert('Username tersebut sudah terdaftar, mohon gunakan username lain !');
	...
	if($password !== $password2){
		echo "<script>
				alert('Password dan konfirmasi password tidak cocok, mohon periksa kembali !');
				window.location = 'tambah-user.php';
			</script>";
		return;
	}
...
if(isset($_POST['update'])){
	...
	if($username !== $userLama){
		if(mysqli_num_rows($cekUsername)){
			echo "<script>
					alert('Username yang baru tidak boleh sama dengan username sebelumnya !');
					window.location = 'index.php';
				</script>";
			return;
		}
...
if(isset($_POST['ganti-password'])){
	...
	if($newPass !== $confPass){
		echo "<script>
				alert('Password gagal diperbarui, dikarenakan konfirmasi password tidak sama !');
