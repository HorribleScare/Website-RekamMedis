...
$sqlrm		= "SELECT *, tbl_pasien.alamat AS alamatpasien FROM tbl_rekamedis INNER JOIN tbl_pasien ON
			tbl_rekamedis.id_pasien = tbl_pasien.id INNER JOIN tbl_user ON
			tbl_rekamedis.id_dokter = tbl_user.userid WHERE no_rm = '$id' ";
		...
		<div class="row">
			<div class="col-lg-6 pe-4">
				<div class="form-group mb-3">
					<label for="no" class="form-label">Nomor Rekam Medis</label>
					<input type="text" name="no_rm" class="form-control" id="no_rm"
					value="<?= $rm['no_rm'] ?>" readonly>
				</div>
				
				<div class="form-group mb-3">
					<label for="tgl" class="form-label">Nomor Perekaman</label>
				...
				<div class="form-group mb-3">
					...
					<div class="input-group mb-3">
						<input type="text" class="form-control" id="pasien_id" name="id" placeholder="ID Pasien"
						value="<?= $rm['id_pasien'] ?>" readonly>
						<button class="btn btn-outline-secondary" type="button" id="cari"
						data-bs-toggle="modal" data-bs-target="#modalPasien"><i class="bi bi-search align-top"></i></button>
					</div>
				</div>
				
				<input type="text" id="namaPasien" class="form-control border-0 border-bottom mb-3" placeholder="Nama Pasien"
				value="<?= $rm['nama'] ?>" readonly>
				<textarea name="" id="alamatPasien" class="form-control border-0 border-bottom" placeholder="Alamat Pasien" rows="1" readonly>
				<?= $rm['alamatpasien'] ?></textarea>
				
				<div class="form-group mb-3">
					<label for="keluhan" class="form-label">Keluhan</label>
					<textarea name="keluhan" id="keluhan" class="form-control" placeholder="Keluhan Pasien Tentang Penyakitnya" rows="1">
					<?= $rm['keluhan'] ?></textarea>
				</div>
			</div>
			...
			<div class="col-lg-6 border-start ps-4">
				...
				<div class="form-group mb-3">
					<label for="obat" class="form-label">Obat (Gunakan koma untuk memisahkan)</label>
						...
						<?php
							...
							<tr>
								...
								<td>
									...
									data-address="<?= $pasien['alamat'] ?>"
									class="btn btn-sm btn-outline-primary cekPasien">
